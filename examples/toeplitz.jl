...
# Triangle
A = BandedMatrix(-2 => Fill(1,∞), 0 => Fill(1/2,∞), 1 => Fill(1/4,∞))
ℓ = λ -> abs(ql(A-λ*I).L[1,1])
...
x = range(-2,2,length=1_000)
    plot(x, real.(ℓ.(x.+eps()im)); label="abs(L[1,1])")    
...
xx = range(0,stop=5,length=100); y = real.([tailql(2,x+0.0im,0.5).L[1,1] for x in xx])


xx = range(0,stop=5,length=100); yy = range(-1,stop=1,length=100);

ℓ = (x,y) -> tailql(2,x+y*im,0.5).L[1,1]

z = real(ℓ.(xx',yy))

contourf(xx,yy,z; nlevels=100)

= real.([tailql(2,x+0.0im,0.5).L[1,1] for x in xx])


plot(xx,y)


x = 1.0; tailql(2+0.0im,x+0.0im,0.5+0.0im)
